...
import {
    fetchItemGroup,saveItemGroup,fetchNextGrpCode, fetchItemGroupById
...
const ItemGroup = () => {
  ...
  const fetchitemgrp = async () => {
    ...
    try {
      ...
      const response = await fetchItemGroup(filters, fieldMapping);
      if (response && Array.isArray(response)) {
        const transformedData = response.map((item) => ({
          id: item.itemG_Id,
          code: item.itemG_Code,
          nameEn: item.itemG_NameEn,
          nameAr: item.itemG_NameAr,
        }));
  ...
  const fetchNextCode = async () => {
    try {
      ...
      setInputValues((prev) => ({ ...prev, code: data.nextNumberGrp }));
    } catch (error) {
  ...
  const handleRowAction = async (rowData) => {
    try {
        ...
        const data = await fetchItemGroupById(id);

        // Update state with the fetched data
        setInputValues({
            id: data.itemG_Id,
            code: data.itemG_Code,
            NameEn: data.itemG_NameEn,
            NameAr: data.itemG_NameAr,
        });
...
//   };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ItemG_Id: inputValues.id || 0, // ID is used for updates, 0 for new entries
        ItemG_Code: inputValues.code,
        ItemG_NameEn: inputValues.NameEn,
        ItemG_NameAr: inputValues.NameAr,
        ItemG_CreatedBy: 1
      ...
      if (response && response.success) {
        Swal.fire("Saved", "Item Group  saved successfully!", "success");
        setShowInputFields(false);
        setInputValues({ id: null, code: "", NameEn: "", NameAr: "" });
        fetchitemgrp(); // Refresh data after saving
      } else {
        Swal.fire("Error", response.statusMessage || "Failed to save item group  .", "error");
      }
    } catch (error) {
    ...
    <div className="page-content">
      <div className="container-fluid">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h4 className="mb-0">Item Group </h4>
          {!showInputFields && (
            <button
