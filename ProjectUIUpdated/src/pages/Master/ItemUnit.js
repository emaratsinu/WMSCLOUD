...
import {
    fetchItemUnit,saveItemUnit,fetchNextUnitCode, fetchUnitById
...
const ItemUnit = () => {
  ...
  const fetchitmunit = async () => {
    ...
    try {
      ...
      const response = await fetchItemUnit(filters, fieldMapping);
      if (response && Array.isArray(response)) {
        const transformedData = response.map((item) => ({
          id: item.unit_Id,
          code: item.unit_Code,
          nameEn: item.unit_NameEn,
          nameAr: item.unit_NameAr,
        }));
  ...
  const fetchNextCode = async () => {
    try {
      ...
      setInputValues((prev) => ({ ...prev, code: data.nextUnitCode }));
    } catch (error) {
  ...
  const handleRowAction = async (rowData) => {
    try {
        ...
        setInputValues({
            id: data.unit_Id,
            code: data.unit_Code,
            NameEn: data.unit_NameEn,
            NameAr: data.unit_NameAr,
        });
...
//   };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        Unit_Id: inputValues.id || 0, // ID is used for updates, 0 for new entries
        Unit_Code: inputValues.code,
        Unit_NameEn: inputValues.NameEn,
        Unit_NameAr: inputValues.NameAr,
      
      };

      const response = await saveItemUnit(payload);
      if (response && response.success) {
        Swal.fire("Saved", "Item Unit  saved successfully!", "success");
        setShowInputFields(false);
        setInputValues({ id: null, code: "", NameEn: "", NameAr: "" });
        fetchitmunit(); // Refresh data after saving
      } else {
    ...
    <div className="page-content">
      <div className="container-fluid">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h4 className="mb-0">Item Unit </h4>
          {!showInputFields && (
            <button
