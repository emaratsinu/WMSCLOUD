...
import {
  fetchNextSaleCode, fetchCustomer, fetchEmployees, FetchParentChldSrls, FetchCheckSrls, saveSuspectedSrl, saveLogSrl, fetchVatDet,fetchSalesList,saveInvoice,fetchInvoiceSrlsById,fetchCardInvNo
...
const SalesToJobCard = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [Customer, setCustomer] = useState([]);
  const [CardInv, setCardInv] = useState([]);
  const [selectedCardInv, setSelectedCardInv] = useState(null);
  const [notes, setNotes] = useState("");
  const [deliveryNotes, setDeliveryNotes] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null);
  const [showSaveButton, setShowSaveButton] = useState(true);
  const [viewLoadingRow, setViewLoadingRow] = useState(null); // Track loading row
  ...
  const [inputValues, setInputValues] = useState({
    code: "", namecode: "", Vat_id: "", Vat_Code: "", Vat_NameEn: "", Vat_NameAr: "", Vat_Per: "",
  });
  ...
  const fetchCardInvoiceNo = async () => {

    try {
      const CardData = await fetchCardInvNo();
      if (CardData) setCardInv(CardData);
    } catch (error) {
      console.error("Error fetching item  Units :", error);
    }
  };
  ...
  const saveSalesInvoice = async () => {
    ...
    try {
        const payload = {
            ...
            sa_CreatedBy: 1,
            sa_JobCardId: selectedCardInv.value,
            sa_BrId: 3,
            sa_RetAmt: 0,
        ...
        if (response.success) {
            ...
            Swal.fire({
                title: "Error!",
                text: "Error saving invoice: " + response.message,
                icon: "error",
                confirmButtonText: "OK"
              });
        }
  ...
  const handleKeyPress = async (e) => {  // Made the function async
    if (e.key === "Enter") {
      ...
      if (!selectedCustomer) {
        Swal.fire({
            title: "Warning!",
            html: "Please select a customer.<br>الرجاء اختيار الزبون",
            icon: "warning",
            confirmButtonText: "OK"
          });
        return false;
                        ...
                        <TabPane tabId={1}>
                          <Form>
                            <Row>
                              ...
                              <Col lg="3">
                                <div className="mb-3">
                                  <Label for="vendorSelect">Job Card No </Label>
                                  <Select
                                    options={CardInv.map((card) => ({
                                      value: card.card_Id,
                                      label: `${card.card_Code} ~ ${card.cGen_Number}`,
                                    }))}
                                    placeholder="Select Job card Number "
                                    classNamePrefix="select2-selection"
                                    isClearable
                                    onChange={(selected) => setSelectedCardInv(selected)}
                                  />
                                </div>
                              </Col>
                            </Row>
